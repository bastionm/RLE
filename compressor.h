...
public:
    ...
    /**
     * Конструктор(поток ввода)
     * @param dst поток ввода
    */
...
private:
    ...
    */
    ...
    void WriteBlock0(char* data, int size) {
        ...
        // ((size - 1) << 1) => 127: 1111111 сдвиг на 1 влево -> 126:1111110
        // + 0 чтбы перевести формат бит в 10-тичный формат.
        unsigned char zero = static_cast<unsigned char>(((size - 1) << 1) + 0);
        dst_.put(zero); // кладем
...
private:
    ...
    int block_size_ = 0; // размер строки c уникальными char
    int repeat_count_ = 0;
    char last_char_;

    char block[max_block_size]; // строка с уникальными char
};
